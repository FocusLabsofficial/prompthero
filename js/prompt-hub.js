...
async function handlePromptSubmission(event) {
    ...
    const promptData = {
        title: formData.get('promptTitle') || document.getElementById('promptTitle')?.value,
        category_slug: formData.get('promptCategory') || document.getElementById('promptCategory')?.value,
        language: formData.get('promptLanguage') || document.getElementById('promptLanguage')?.value,
        prompt_text: formData.get('promptText') || document.getElementById('promptText')?.value,
        description: formData.get('promptDescription') || document.getElementById('promptDescription')?.value,
        tags: (formData.get('promptTags') || document.getElementById('promptTags')?.value || '').split(',').map(tag => tag.trim()).filter(tag => tag)
    };
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast(errorData.error || 'Failed to submit prompt', 'error');
        }
    } catch (error) {
...
function scrollToCategories() {
    const categoriesSection = document.querySelector('.component-type-filters');
...
function showTopRated() {
    // Set filter to popular (top rated)
    setUnifiedFilter('popular');
    showToast('Showing top rated prompts', 'info');
}
