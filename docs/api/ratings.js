...
export default async function handler(request, response) {
  try {
    ...
    if (method === 'POST') return await createRating(request, response);
    if (method === 'GET') return await getRatings(request, response);
    if (method === 'PUT') return await updateRating(request, response);
...
async function createRating(request, response) {
  ...
  if (!prompt_id || !rating || rating < 1 || rating > 5) {
    return response.status(400).json({ error: 'Missing or invalid required fields: prompt_id, rating (1-5)' });
  }
  if (!user_identifier) {
    return response.status(400).json({ error: 'User identifier is required' });
  }
  try {
    ...
    if (existing.rows.length > 0) {
      const result = await pool.query(
        `UPDATE ratings SET rating = $1, review = $2, updated_at = NOW()
         WHERE prompt_id = $3 AND user_identifier = $4 RETURNING *`,
        [rating, review || null, prompt_id, user_identifier]
      );
      await updatePromptAverageRating(prompt_id);
      return response.status(200).json({ message: 'Rating updated successfully', rating: result.rows[0] });
    } else {
      const result = await pool.query(
        `INSERT INTO ratings (prompt_id, rating, review, user_identifier, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING *`,
        [prompt_id, rating, review || null, user_identifier]
      );
      await updatePromptAverageRating(prompt_id);
      return response.status(201).json({ message: 'Rating created successfully', rating: result.rows[0] });
    }
  } catch (error) {
...
async function getRatings(request, response) {
  ...
  if (!prompt_id) return response.status(400).json({ error: 'prompt_id is required' });
  try {
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const countResult = await pool.query('SELECT COUNT(*) as total FROM ratings WHERE prompt_id = $1', [prompt_id]);
    const total = parseInt(countResult.rows[0].total);
    const ratingsResult = await pool.query(
      `SELECT id, rating, review, user_identifier, created_at, updated_at
       FROM ratings WHERE prompt_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
...
async function updatePromptAverageRating(promptId) {
  try {
    ...
    await pool.query('UPDATE prompts SET rating = $1, rating_count = $2 WHERE id = $3', [avgRating, totalRatings, promptId]);
  } catch (error) {
